...
var App = /** @class */ (function () {
    ...
    App.prototype.processQueueTask = function (queueBulk, cb) {
         ...
         * @param fileData
         */
        function autoOrientThumb(fileData) {
            logger_1.Logger.info('Attempting to auto-orient file:' + fileData.thumbPath + ' with orientation value of:' + fileData.orientation);
            return new Promise(function (resolve, reject) {
                //gm(fileData.path).rotate()
                if (!_.isEmpty(fileData.orientation)) {
                    logger_1.Logger.info('Orientation found of:' + fileData.orientation);
                    if (fileData.orientation === ExifOrientation.TOP_LEFT + '') {
                        ...
                        logger_1.Logger.info('Orientation of:' + fileData.orientation + ' found. There is a need to auto orient.');
                        var rotationAngle = 0;
                        ...
                        if (rotationAngle > 0) {
                            ...
                            gm(fileData.thumbPath).rotate('white', rotationAngle)
                                .write(fileData.thumbPath, function (err) {
                                if (!_.isEmpty(err)) {
                                    reject(err);
                                }
                                else {
        ...
        function createThumbnail(fileData) {
            ...
            return new Promise(function (resolve, reject) {
                fileData.thumbPath = path.join(path.dirname(fileData.path), path.basename(fileData.path, path.extname(fileData.path)) + '_THUMB' + path.extname(fileData.path));
                logger_1.Logger.info('Creating thumbnail:' + fileData.thumbPath);
                ...
                gm(fileData.path).identify(function (err, props) {
                    ...
                    if (!_.isEmpty(props)) {
                        ...
                        if (props.hasOwnProperty('size')) {
                            ...
                            gm(fileData.path).thumb(props.size.width / 2, props.size.height / 2, fileData.thumbPath, conf_json_1.default.thumbnailSettings.quality, function (err) {
                                if (!_.isEmpty(err)) {
                                    reject(err);
                                }
                                else {
